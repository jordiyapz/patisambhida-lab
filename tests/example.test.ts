...
const testList: Array<[string, string, Array<number | null>, boolean?]> = [
  ...
  ["GADBCDFGFB", "ABCDEFG", [null, 0, null, 1, 2, 3, 5, 6, null, null], true],
];
...
test("recursiveWindowPeek (Realcase)", () => {
  const origin = [
    '"atītesu',
    "anāgatesu",
    "cāpi,",
  ...
  const result = recursiveWindowPeek({ target, current: origin, debug: false });
